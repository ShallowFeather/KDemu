...
public:
	...
	static uint64_t HeapAlloc(uc_engine* uc, uint64_t size, bool show, void* user_data);
	...
	static void IoGetRelatedDeviceObject(uc_engine* uc, uint64_t address, uint32_t size, void* user_data);
	static void IoBuildDeviceIoControlRequest(uc_engine* uc, uint64_t address, uint32_t size, void* user_data);
	static void KiDeliverApc(uc_engine* uc, uint64_t address, uint32_t size, void* user_data);
...
public:
	std::map<std::string, void(*)(uc_engine*, uint64_t, uint32_t, void*)> NtfuncMap = {
		...
		{ "IoGetRelatedDeviceObject",Emulate::IoGetRelatedDeviceObject },
		{ "IoBuildDeviceIoControlRequest",Emulate::IoBuildDeviceIoControlRequest },
		{"KiDeliverApc",Emulate::KiDeliverApc }
	};
	...
	static void hook_block_track(uc_engine* uc, uint64_t address, uint32_t size, void* user_data);
	static void hook_edge_generated(uc_engine* uc, uc_tb* cur, uc_tb* prev, void* user_data);
	...
	static void hook_access_objectH(uc_engine* uc, uint64_t address, uint32_t size, void* user_data);
	...
	static void hook_Symbol_func(uc_engine* uc, std::vector<ExportFunctionInfo> exports, void* user_data, std::string funcName, void(*func)(uc_engine*, uint64_t, uint32_t, void*));
